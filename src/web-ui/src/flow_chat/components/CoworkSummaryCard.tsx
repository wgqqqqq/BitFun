...
export interface CoworkSummaryCardSummary {
...
export function CoworkSummaryCard(props: { summary: CoworkSummaryCardSummary }): React.ReactElement {
  const { summary } = props;
  ...
  return (
    <Card className="cowork-summary-card" variant="subtle" fullWidth padding="medium">
      <CardHeader
        ...
        subtitle={summary.phaseHint ? <div className="cowork-summary-card__subtitle">{summary.phaseHint}</div> : undefined}
        extra={headerExtra}
      />
      <CardBody>
        {summary.total > 0 ? (
        ...
        {summary.waitingTaskId && summary.questions?.length ? (
          <div className="cowork-summary-card__section">
            ...
            <div className="cowork-summary-card__hint">
              Reply in chat with your answers (one per line).
            </div>
          </div>
        ) : null}
      </CardBody>
