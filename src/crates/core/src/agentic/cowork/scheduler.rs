...
pub async fn run_scheduler_loop(
    ...
    loop {
        ...
        loop {
            let done =
                tokio::time::timeout(std::time::Duration::from_millis(0), join_set.join_next()).await;
            match done {
                Ok(Some(Ok(()))) => continue,
                Ok(Some(Err(e))) => {
                    warn!(
                        "Cowork task future failed: cowork_session_id={}, error={}",
                        cowork_session_id, e
                    );
                    continue;
                }
            ...
            if let Some(mut t) = tasks_by_id.get(task_id).cloned() {
                    ...
                    && deps_failed(&t, &tasks_by_id).is_some()
                {
                    if t.state != CoworkTaskState::Blocked {
                        let dep_id =
                            deps_failed(&t, &tasks_by_id).unwrap_or_else(|| "unknown".to_string());
                        t.state = CoworkTaskState::Blocked;
                        t.error = Some(format!(
                            "Blocked: dependency '{}' failed or was cancelled",
                            dep_id
                        ));
                        t.updated_at_ms = chrono::Utc::now().timestamp_millis();
                        manager.update_task(cowork_session_id, t.clone()).await?;
        ...
        for task_id in &session.task_order {
            ...
            let Some(t0) = tasks_by_id.get(task_id).cloned() else {
                continue;
            };
                ...
                match result {
                    Ok(r) => {
                        ...
                        if let Err(e) = manager
                            .update_task(&cowork_session_id_owned, task.clone())
                            .await
                        {
                            warn!(
